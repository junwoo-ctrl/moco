...
def get_sample_tensor_data(is_gpu: bool = True):
    ...
    print(f"sample iamge path: {sample_image_filepath} \n")
    return sample_image_tensor


def execute_inference(model: nn.Module, data: torch.Tensor):

    model.eval()
    hat = model(data).to('cpu')
    _, pred = hat.topk(5, 1, True, True)
    pred = pred.t()
    return pred
...
def torch_inference_with_gpu(*, verbose: bool = True):

    fix_environment()
    moco_v2: nn.Module = load_pretrained_model_cuda(is_gpu=True)
    sample_image_tensor = get_sample_tensor_data(is_gpu=True)

    print("[  model summary  ]")
    print(summary(moco_v2))
    print(f"sample tensor dimension: {sample_image_tensor.shape}. \n")

    print("[  single inference  ] ")
    preds = execute_inference(model=moco_v2, data=sample_image_tensor)
    imagenet_labels = get_labels()
    for idx, pred in enumerate(preds):
        target_class_index = int(pred[0])
        print(f"top {idx} predict: {imagenet_labels[target_class_index]}")

    print("[  inference for 10000 iterations  ]")
    start = arrow.utcnow()
    for i in range(0, 10000):
        execute_inference(model=moco_v2, data=sample_image_tensor)
    end = arrow.utcnow()
    verbose_interval(start=start, end=end)


def torch_inference_with_cpu(*, verbose: bool = True):
    fix_environment()
    moco_v2: nn.Module = load_pretrained_model_cuda(is_gpu=False)
    sample_image_tensor = get_sample_tensor_data(is_gpu=False)
    print("[  single inference  ] ")
    preds = execute_inference(model=moco_v2, data=sample_image_tensor)
    imagenet_labels = get_labels()
    for idx, pred in enumerate(preds):
...
if __name__ == "__main__":
    run({
        "gpu_inference": torch_inference_with_gpu,
        "cpu_inference": torch_inference_with_cpu,
    })